...
if __package__ is None:
    ...
    from casas_data_parse import SensorData, get_data, time_int, write_header
...
class SensorSeries:
    ...
    def _is_episode_end(self, i, **kwargs):
        ...
        look_ahead : INT
            ...
            THE PATTERN EPISODE IS TRULY ENDED.
        ...
        # for episodes based solely on timestamps between certain hours of the day
        # namepatterns will be all sensors, only thing that determines end of episode is timestamp
        # check if time stamp for current episode is less than cutoff 
        if self.namepatterns_tag == 'all':
            if self.nighttime is True and curdatetime.time() < self.morning_time:
                return False
            elif self.nighttime is False and curdatetime.time() < self.evening_time:
                return False
            else:
                return True
        ...
        #get kwarg indicating if data only includes "ON" sensor events
        on_only = kwargs.get('on_only', False)
        ...
        if i == len(self.data) - 1:
            ...
            if include_end:
                ...
                pass
            return True
        # are any target sensors still "ON"? 
        # if on_only then set treat it as if all sensors are OFF (empty list)
        if on_only:
            targetsON = []
        else:
            # creates list of sensor names whose last observed message was ON
        ...
        timesince = (curdatetime - lastdatetime) < datetime.timedelta(minutes=2)
        if bool(targetsON) and timesince:
            return False
        else:
            for n in range(look_ahead+1):
                event=self.data[i+n]
            ...
            if ooh:
                avg_event_freq = (self.data[i+look_ahead]['DateTime'] - lastdatetime)/(look_ahead/2)
    ...
    def find(self, **kwargs):
        ...
        **kwargs : DICT, optional
        ...
        Other Parameters
        ----------------
        look_ahead, include_end : OPTIONAL KWARGS FOR PASSING TO :func:`~pattern_search.SensorSeries._is_episode_end` SPECIFYING HOW TO DETERMINE THE END OF AN EPISODE
                    DEFAULTS ARE look_ahead=2 AND include_end=False
...
class Bedroom(SensorSeries):
    ...
    def find(self, morning_time, evening_time, nighttime=False, **kwargs):
        ...
        SensorSeries.find(self, morning_time=morning_time, evening_time=evening_time, nighttime=nighttime, look_ahead=2, **kwargs)
...
class OutofHome(SensorSeries):
    ...
    def find(self, min_duration=None, **kwargs):
        ...
        #e.g. if morning_time=2200 and evening_time=0600 and nighttime==True,
        #then only sensor events occuring between 2200 and midnight and midnight and 0600 will be included
        kwargs['look_ahead'] = kwargs.get('look_ahead', 20)
...
if __name__ == "__main__":
    ...
    file_to_open = data_dir / pathlib.Path("tm000.20160210-20160212_20220819.192044.txt")
    sensordata = SensorData(file_to_open, ignore={'Message':['OFF', 'CLOSE']})
    bathroomAarea = SensorSeries(sensordata, ["BathroomAArea"])
    bathroomAarea.find(look_ahead=2, on_only=True)
    """
    #data_folder = 'tm015\\'
